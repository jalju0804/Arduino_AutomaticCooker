...
MAX6675 thermocouple (pinTemps_SCK, pinTempsCS_1, pinTemps_SO); // 온도센서
...
int readMux_eachSensor (const byte which) // switch 읽기
{
...
byte readMux_AllChannel() // mux 상태 return
{
...
byte readMux_SwChannel() // switch 상태 return
{
  //--no txd digitalWrite(pin_muxCE, LOW);  
...
void Mux_Selection_Doing(int MuxSelection,int state) // 0~7 input 220815
{
	digitalWrite(pin_muxAddress_A,(MuxSelection & 1) ? HIGH :LOW);
	digitalWrite(pin_muxAddress_B,(MuxSelection & 2) ? HIGH :LOW);
	digitalWrite(pin_muxAddress_C,(MuxSelection & 4) ? HIGH :LOW);
	
	digitalWrite(pin_muxOnCH,(state) ? HIGH : LOW);
	return;
}

// Read Tempe. Sensor ( Water1 / Recipe2 Box )....
float read_TemperatureSensor(const byte which) //온도 센서 읽기
{
  //float tempe;
  float tempe = -1;
  if (which == 1)
...
int read_AmbientSensor(const byte pinNum) // 근접 센서 읽기
{
  int ambientValue;
  //int ambientValue;  
...
void Delay_And_Check(int delay_Time, int chk_mode) //운행 x 대기시에 물,육수의 온도,양 체크
{
	byte byteMuxValue = 0;
		...
		for (int i = 0; i < delay_Time; i++)
		{
			switch(chk_mode)
			{
				...
				case 3:	// ------ None....Motor Down move 중....
					...
					break;	
				case 7:	// Only Steam Button Pressed ==> 스팀 투입 중....
					control_RecipeBox (10); // temp.
					control_WaterBox (30); // press. ++20201117
					byteMuxValue = readMux_SwChannel();
					if (byteMuxValue != 4)
						steamBtnPress_Count++;
					else
						steamBtnPress_Count = 0;
					if (steamBtnPress_Count > 3) // 3초 이상, 스팀버튼이 연속 "안"-눌려진 상태에서 멈춤.....
						fLoopBreak = true;					
					break;									
				default:
					break;
...
// Water Box Control.....
void control_WaterBox(int addMax_value) // 물 양 체크 *물이랑 육수랑 다른거??*
{
	//--no txd digitalWrite(pin_muxCE, LOW);	  
...
// Recipe Box Control.....
void control_RecipeBox(int addMax_value) //육수 넣기 전 온도,양 준비
{
	//--no txd digitalWrite(pin_muxCE, LOW);	  
    // check Water Level Sensor.....
#if defined (__MY_DEBUG_LOG__)                        
...
void motor_move(const int step) // 용기 올리는 모터 동작제어
{
  int Speed;
...
void lms_start_ButtonPress(int joriMode)
{
	...
	switch(joriMode)
	{
		case large_ButtonPressed_mode: // SSalGuksu..........
			Delay_And_Check(19, 0); // 19 sec. ( 450ml add... ) recipe-water
			break;
		case middle_ButtonPressed_mode:
			Delay_And_Check(17, 0);  // 17 sec. ( 410ml add... )
			break;
		case small_ButtonPressed_mode:
			Delay_And_Check(10, 0);  // 10 sec. ( 240ml add... )
			break;
	}
	// 육수 투입 정지:
	...
	switch(joriMode)
	{
		...
		case large_ButtonPressed_mode:
			Delay_And_Check(55, 2); // 60 sec. steam
			break;
		case middle_ButtonPressed_mode:
			Delay_And_Check(45, 2);  // 50 sec.
			break;
		case small_ButtonPressed_mode:
			Delay_And_Check(45, 2);  // 50 sec.
			break;
	...
	// 15 sec.  대기......
	// 전체 15초인데,   (모터 움직이는 시간-Top=>Bottom : 대략 20초) 모터 움직이는 시간을 감안해서... 9초만 Holding....	
	//Delay_And_Check(9, 1);		
	// 전체 15초인데,   (모터 움직이는 시간-Top=>Bottom : 대략 20초) 모터 움직이는 시간을 감안해서... 9초만 Holding....	
	Delay_And_Check(15, 1);	 // 밑에서, 젓가락으로 한번 젓는다고 해서  15초 기다림......
...
void steam_ButtonPress()
{
	...
	//--Delay_And_Check(15, 2);	// 15 sec.
	Delay_And_Check(30, 7); // Input steam for steam button pressed......... : Max. 30 sec.....
